...
def build_report(dataset_root: Path) -> dict[str, Any]:
    ...
    collector_policy_id_col = "complementary_info.collector_policy_id"
    collector_source_col = "complementary_info.collector_source"
    episode_index_col = "episode_index"
    intervention_frames = 0
    intervention_episode_count = 0
    collector_policy_ids: list[str] = []
    ...
    if collector_policy_id_col in data_schema_names and actual_frame_count > 0:
        collector_policy_ids = [
            str(v)
            for v in data_dataset.to_table(columns=[collector_policy_id_col])[collector_policy_id_col].to_pylist()
            if v is not None and str(v)
        ]

    if collector_source_col in data_schema_names and actual_frame_count > 0:
        collector_sources = [
            str(v)
            for v in data_dataset.to_table(columns=[collector_source_col])[collector_source_col].to_pylist()
            if v is not None and str(v)
        ]
    ...
    return {
        ...
        "collector_metrics": {
            "has_collector_policy_id_column": collector_policy_id_col in data_schema_names,
            "has_collector_source_column": collector_source_col in data_schema_names,
            "unique_collector_policy_ids": sorted(set(collector_policy_ids)),
            "unique_collector_sources": sorted(set(collector_sources)),
        },
    }
...
def format_text_report(report: dict[str, Any]) -> str:
    ...
    collector = report["collector_metrics"]
    lines.append("[Collector Metrics]")
    lines.append(f"- has collector_policy_id field: {collector['has_collector_policy_id_column']}")
    lines.append(f"- has collector_source field: {collector['has_collector_source_column']}")
    lines.append(f"- unique collector sources: {collector['unique_collector_sources']}")
    lines.append(f"- unique collector policy ids: {collector['unique_collector_policy_ids']}")
    lines.append("")
