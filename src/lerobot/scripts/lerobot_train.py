...
def update_policy(
    ...
    Args:
        ...
        sample_weights_provider: Optional sample-weight provider for weighted BC.
    ...
    batch_weights = None
    batch_weight_stats = None
    provider_prefix = None
    if sample_weights_provider is not None:
        batch_weights, batch_weight_stats = sample_weights_provider.compute_batch_weights(batch)
        provider_prefix = getattr(sample_weights_provider, "log_prefix", "weights")
    ...
    with accelerator.autocast():
        ...
        if batch_weights is not None:
            ...
            # Apply weighted BC: L = Σ(w_i * l_i) / (Σw_i + ε)
            epsilon = 1e-6
            loss = (per_sample_loss * batch_weights).sum() / (batch_weights.sum() + epsilon)
            for key, value in (batch_weight_stats or {}).items():
                output_dict[f"{provider_prefix}_{key}"] = value
        else:
...
def train(
    ...
    if cfg.use_rabc:
        ...
        logging.info(f"Loading ICCQ weights for QBC from {cfg.qbc_weight_path}")
        sample_weights = QBCWeights(
            weights_path=cfg.qbc_weight_path,
            beta=cfg.qbc_beta,
            clip_min=cfg.qbc_clip_min,
            clip_max=cfg.qbc_clip_max,
            weight_column=cfg.qbc_weight_column,
            advantage_column=cfg.qbc_advantage_column,
            device=device,
        )
    ...
    for _ in range(step, cfg.steps):
        ...
        train_tracker, output_dict = update_policy(
            ...
            sample_weights_provider=sample_weights,
        )
        ...
        if is_log_step:
            ...
            if wandb_logger:
                ...
                if sample_weights is not None:
                    provider_prefix = getattr(sample_weights, "log_prefix", "weights")
                    provider_stats = sample_weights.get_stats()
                    wandb_log_dict.update(
                        {f"{provider_prefix}_{key}": value for key, value in provider_stats.items()}
                    )
