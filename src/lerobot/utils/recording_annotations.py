...
COLLECTOR_SOURCE_POLICY = "policy"
COLLECTOR_SOURCE_HUMAN = "human"
VALID_COLLECTOR_SOURCES = {COLLECTOR_SOURCE_POLICY, COLLECTOR_SOURCE_HUMAN}
...
def resolve_collector_policy_id(
    *,
    ...
    """Resolve frame-level `collector_policy_id` as policy provenance, not action source."""
    if policy_id:
        return policy_id
    return human_id


def resolve_collector_source(
    *,
    intervention_enabled: bool,
    is_intervention: bool,
    selected_from_policy: bool,
) -> str:
    """Resolve frame-level execution source independently from policy provenance."""
    if intervention_enabled:
        return COLLECTOR_SOURCE_HUMAN if is_intervention else COLLECTOR_SOURCE_POLICY
    return COLLECTOR_SOURCE_POLICY if selected_from_policy else COLLECTOR_SOURCE_HUMAN
